...
export class GitSeeHandler {
  ...
  private async getRepoIcon(owner: string, repo: string): Promise<string | null> {
    ...
    try {
      ...
      for (const subdir of subdirs) {
        const subdirExists = rootContents.data.find((item: any) => 
          item.name === subdir && item.type === 'dir'
        );
        ...
        if (subdirExists) {
          try {
            ...
            if (Array.isArray(subdirContents.data)) {
              ...
              iconFiles.push(...subdirIcons.map((f: any) => ({ ...f, path: `${subdir}/${f.name}` })));
            }
          } catch (error) {
